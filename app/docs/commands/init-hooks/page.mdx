...
The `--threshold` flag is used to block commits which exceed a certain danger level. 
The `--block-critical` will block commits which include changes flagged as High risk or critical.
