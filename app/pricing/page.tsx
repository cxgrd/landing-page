      ...
      <header className="sticky top-0 z-30 border-b border-white/10 bg-[#05070f]/80 backdrop-blur-xl">
        <div className="mx-auto flex w-full max-w-6xl items-center justify-between px-6 py-4">
          ...
          <nav className="hidden items-center gap-6 text-sm text-slate-300 md:flex">
            ...
            <a href="/pricing" className="transition-colors hover:text-white font-medium text-white">
              Pricing
            </a>
            <a 
          ...
          <a href={githubLink} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-lg border border-white/15 px-4 py-2 text-sm font-medium text-slate-200 transition-all duration-300 hover:border-blue-300/60 hover:text-white hover:shadow-lg hover:shadow-blue-500/20">
      ...
      <main className="mx-auto w-full max-w-6xl px-6 pb-24 pt-14 sm:pt-20">
        <section className="mx-auto max-w-4xl text-center">
          ...
          <p className="mx-auto mt-6 max-w-3xl text-pretty text-base leading-relaxed text-slate-300 sm:text-lg">
            Start free and scale as you grow. No credit card required for the free tier. Pay only for what you use when you're ready to unlock advanced features.
          </p>
        </section>
        ...
        <section className="mt-16 grid gap-8 lg:grid-cols-2 xl:grid-cols-4">
          {pricingTiers.map((tier) => (
            <article
              key={tier.name}
              className={`feature-card glass-surface rounded-2xl p-8 flex flex-col ${
                tier.highlight ? "border-blue-400/40 shadow-lg shadow-blue-500/20 xl:col-span-1 xl:scale-105" : ""
              }`}
            >
              <div className="mb-6">
              ...
              <div className="mb-6">
                ...
                {tier.period && (
                  <span className="ml-2 text-sm text-slate-400">/ {tier.period}</span>
                )}
              </div>

              <button
                className={`mb-8 rounded-xl py-3 px-4 font-semibold text-sm transition-all duration-300 ${
                  tier.ctaVariant === "primary"
                    ? "bg-gradient-to-r from-blue-500 via-indigo-500 to-violet-500 text-white hover:-translate-y-0.5 hover:from-blue-400 hover:via-indigo-400 hover:to-violet-400 hover:shadow-lg hover:shadow-blue-500/25"
                    : "border border-white/15 text-slate-200 hover:border-blue-300/60 hover:text-white hover:shadow-lg hover:shadow-blue-500/20"
                }`}
              >
                {tier.cta}
              </button>
